...
function createPublicationElement(entry) {
  ...
  if (entry.abstract) {
    ...
    abstractBtn.onclick = () => {
      ...
      abstractBtn.textContent = abstractDiv.classList.contains("show")
        ? "Hide Abstract"
        : "Abstract";
    };
  ...
  if (entry.arxiv) {
    links.appendChild(
      createLink("arXiv", `https://arxiv.org/abs/${entry.arxiv}`),
    );
  }
  ...
  if (entry.slides) {
    const slidesUrl = entry.slides.startsWith("http")
      ? entry.slides
      : `../asset/pdf/${entry.slides}`;
    links.appendChild(createLink("Slides", slidesUrl));
  }

  // Poster link - can be full URL or relative path to assets folder
  if (entry.poster) {
    const posterUrl = entry.poster.startsWith("http")
      ? entry.poster
      : `../asset/pdf/${entry.poster}`;
